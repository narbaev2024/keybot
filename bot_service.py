...
conn = sqlite3.connect('keybot.db', check_same_thread=False)
cursor = conn.cursor()
...
def remove_certificate(message):
    ...
    cursor.execute("DELETE FROM certificates WHERE user_id = ? AND certificate_name = ?", (user_id, name))
    conn.commit()
    ...
    if step == 1:
        ...
        if 'update' in user_states[user_id]:
            try:
                ...
                update_certificate(user_id, certificate_name, expiration_date.strftime('%Y-%m-%d %H:%M'))
        ...
        try:
            ...
            add_certificate(user_id, message.from_user.full_name, user_states[user_id]['certificate_name'],
                            user_states[user_id]['certificate_key'], expiration_date.strftime('%Y-%m-%d %H:%M'))
...
def add_certificate(user_id, user_name, certificate_name, certificate_key, expiration_date):
    cursor.execute(
        "INSERT INTO certificates (user_id, user_name, certificate_name, certificate_key, expiration_date) VALUES (?, ?, ?, ?, ?)",
...
def update_certificate(user_id, certificate_name, expiration_date):
    cursor.execute("UPDATE certificates SET expiration_date = ? WHERE user_id = ? AND certificate_name = ?",
...
def check_certificates(message):
    ...
    try:
        cursor.execute("SELECT certificate_name, certificate_key, expiration_date FROM certificates WHERE user_id = ?",
...
def send_reminders():
    ...
    cursor.execute("SELECT user_id, certificate_name, expiration_date FROM certificates WHERE expiration_date <= ?",
                   (threshold_time.strftime('%Y-%m-%d %H:%M'),))
    rows = cursor.fetchall()
